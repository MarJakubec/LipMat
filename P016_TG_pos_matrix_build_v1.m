% TG Positive Fragmentation Library
% This script will build TG_pos_Matrix.mat file which will contain
% prediction of TG lipid fragmentation 
% Warning!!! TG have three fatty acid chains, high number of combination
...
fragments_non{1} = {'Precursor ion [M+H]+ with loss of H2O',...
    ...
    'FA3 as ketene','FA1 ketene + glycerol',...
    'FA2 ketene + glycerol','FA3 ketene + glycerol',...
    ...
    'FA2 + glycerol with loss of H2O','FA3 + glycerol with loss of H2O'}; 
...
for i = 1:(size(FA_list,1))
        ...
        P_matrix(m).structure(:,[1:6]) = FA_list(i,:);
        ...
        P_matrix(m).mass = FA1_ketene + FA2_ketene + FA3_ketene + glycerol_residue+ water + adduct_mass(o);
        ...
        P_matrix(m).frag(5) = FA1_ketene + H_pos; %FA1 as ketene
        P_matrix(m).frag(6) = FA2_ketene + H_pos; %FA2 as ketene
        P_matrix(m).frag(7) = FA3_ketene + H_pos; %FA3 as ketene
        
        P_matrix(m).frag(8) = FA1_ketene + glycerol_residue + H_pos; %lFA1 and glycerol
        P_matrix(m).frag(9) = FA2_ketene + glycerol_residue + H_pos; %lFA2 and glycerol
        P_matrix(m).frag(10) = FA3_ketene + glycerol_residue + H_pos; %lFA3 and glycerol
        
        P_matrix(m).frag(11) = FA1_ketene - water + H_pos; %FA1 as ketene with loss of water
        P_matrix(m).frag(12) = FA2_ketene - water + H_pos; %FA2 as ketene with loss of water
        P_matrix(m).frag(13) = FA3_ketene - water + H_pos; %FA3 as ketene with loss of water
        
        P_matrix(m).frag(14) = FA1_ketene + glycerol_residue + H_pos - water; %lFA1 and glycerol - water
        P_matrix(m).frag(15) = FA2_ketene + glycerol_residue + H_pos - water; %lFA2 and glycerol - water
...
lipid_name = ('TG');
save('TG_pos_Matrix.mat','P_matrix','lipid_name','polarity');
