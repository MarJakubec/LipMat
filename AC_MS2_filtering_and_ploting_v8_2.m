...
if isempty(MS2_hits)
    ...
    for i = 1:size(finds,1)
        ...
        elseif strcmp(finds{i,3},'TG')
        str_finds{i} = sprintf('%s %s %d:%d/%d:%d/%d:%d',finds{i,3},finds{i,2},finds{i,1}(1),...
            finds{i,1}(2),finds{i,1}(3),finds{i,1}(4),finds{i,1}(5),finds{i,1}(6));
        elseif strcmp(finds{i,3},'MG')
        str_finds{i} = sprintf('%s %s %d:%d',finds{i,3},finds{i,2},finds{i,1}(1),...
            finds{i,1}(2));  
        else
...
print(sprintf('%s/LIPID_ID_%d.png',fullfile(pwd, newfolder),i),'-dpng');
 ...
 clearvars -except o files cut_off  
end
clear all 
close all 
['No errors!']
