...
if ~(strcmp(un_lipids{k},'CL') || strcmp(un_lipids{k},'SM')...
        || strcmp(un_lipids{k},'TG') || strcmp(un_lipids{k},'MG'))
...
elseif isfield(MS1_to_plot,'FA3') && ~isfield(MS1_to_plot,'FA4')
FAList = [{MS1_to_plot.FA1} {MS1_to_plot.FA2} {MS1_to_plot.FA3}];
elseif isfield(MS1_to_plot,'FA1') && ~isfield(MS1_to_plot,'FA2')
FAList = [{MS1_to_plot.FA1}]; 
else
...
for i = 1:size(FAList,1)
    ...
    if ~isempty(Index)
        for j = 1:size(Index,1)
            FAList{i,2} = FAList{i,2} + MS1_to_plot(Index(j)).area/3;          
        end
    end
    Index = find(contains(FA2,FAList{i}));
    if ~isempty(Index)
...
for i = 1:size(FAList,1)
    ...
    if ~isempty(Index)
        for j = 1:size(Index,1)
            FAList{i,2} = FAList{i,2} + MS1_to_plot(Index(j)).area;          
        end
    end
end
else
